...
# --------------------------------- E.A. ---------------------------------------------- #
# To link cppkafka with API, link it with:
# 1. cppkafka
# 2. rdkafka

### cppkafka directory (Link cppkafka with PhysiBoSS)
# CPPKAFKA := cppkafka
# CPPKAFKA_SRC := $(CPPKAFKA)/src
# CPPKAFKA_LIB := -L$(CUR_DIR)/$(CPPKAFKA)/build/src/lib -lcppkafka
# CPPKAFKA_INC := -I$(CUR_DIR)/$(CPPKAFKA)/include/cppkafka

### rdkafka directory (Link rdkafka with PhysiBoSS)
# RDKAFKA := $(CPPKAFKA)/librdkafka
# RDKAFKA_SRC := $(RDKAFKA)/src
# RDKAFKA_LIB := -L$(CUR_DIR)/$(RDKAFKA)/install/lib -lrdkafka
# #RDKAFKA_LIB := -L$(CUR_DIR)/$(RDKAFKA_SRC) -lrdkafka
# RDKAFKA_INC := -I$(CUR_DIR)/$(RDKAFKA)/install/include/librdkafka

#---------- Folders to look in
# SEARCH for %.o, %.cpp and %.h files to the specified paths, respectively.
vpath %.o     $(BUILD_DIR)
...
#----- Mine
#vpath %.cpp 	  $(SRC_DIR)/cppkafka/src	
#vpath %.cpp 	  $(SRC_DIR)/cppkafka/src/utils
#vpath %.h 	      $(SRC_DIR)/cppkafka/include/cppkafka
#vpath %.h 	      $(SRC_DIR)/cppkafka/include/cppkafka/utils
#vpath %.h 	      $(SRC_DIR)/cppkafka/include/cppkafka/detail
...
FlagsR := $(RELEASE) -march=native -s -fomit-frame-pointer -mfpmath=both -fopenmp -m64 -std=c++0x
FlagsP := $(PROFILE)
...
physiboss: $(BIN_DIR) $(MAIN)
	$(COMPILE_COMMAND) $(INC) -o $(BIN_DIR)/PhysiBoSS ./src/main/PhysiBoSS.cpp $(BUILD_DIR)/*.o $(LIB) -lcppkafka -lrdkafka -ldl

create: $(BIN_DIR) $(MAIN2) 
	$(COMPILE_COMMAND) -o $(BIN_DIR)/PhysiBoSS_CreateInitTxtFile ./src/main/createTxt.cpp $(BUILD_DIR)/*.o $(LIB) -lcppkafka -lrdkafka -ldl

plot: $(BIN_DIR) $(MAIN3) 
	$(COMPILE_COMMAND) $(INC) -o $(BIN_DIR)/PhysiBoSS_Plot ./src/main/plotTxt.cpp $(BUILD_DIR)/*.o $(LIB) -lcppkafka -lrdkafka -ldl
